...
import skillApi from '../api/skillApi'; // dùng cho modal tìm kỹ năng
...
const PostJob = () => {
  ...
  const handlePost = async () => {
    ...
    // ✅ Đính kèm yêu cầu kỹ năng – data giống trang SkillsPage
    formData.append('skills', JSON.stringify(reqSkills));
    // Ví dụ body backend sẽ nhận kiểu:
    // skills: [{skill_id, name, level, years_exp}, ...]

    try {
      setLoading(true);
      await jobApi.create(formData); // đảm bảo axios không set 'Content-Type: application/json'
      setShowSuccess(true);
        ...
        <AddSkillRequirementModal
          ...
          onAdded={(row) => {
            setReqSkills(prev => {
              ...
              return has
                ? prev.map(x => x.skill_id === row.skill_id ? row : x)
                : [row, ...prev];
            });
...
/** ------------ Modal chọn kỹ năng + Level/Years ------------ */
function AddSkillRequirementModal({ onClose, onAdded }) {
  ...
  const addOne = async (opt) => {
    // Không gọi API upsertMine vì đây là yêu cầu của Job, không phải kỹ năng của "tôi"
    // Ta chỉ thêm vào danh sách reqSkills của bài đăng
